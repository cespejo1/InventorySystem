...
public class ModifyProductController implements Initializable {
    ...
    private void addButtonAction(ActionEvent event) {
          ...
          selectedItem = tableView1.getSelectionModel().getSelectedItem();
            ...
            modifiedProduct.addPart(selectedItem);
    ...
    private void deleteButtonAction(ActionEvent event) {
        ...
        if(modifiedProduct.getPartsList().size() > 1){
            
            //confirm with user
            Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
            alert.setTitle("Confirmation Dialog");
            alert.setHeaderText("Are you sure you want to cancel?");
            alert.setContentText("Any changes you have made will not be saved");
            Optional<ButtonType> result = alert.showAndWait();
                ...
                if (result.get() == ButtonType.OK){
                    ...
                    modifiedProduct.getPartsList().remove(selectedItem);
                    tableView2.setItems( associatedTableData );
                } 
    ...
    private void saveButtonAction(ActionEvent event) throws IOException {
        ...
        if(allFieldsAreFilled){
            ...
            modifiedProduct.setInStock(Integer.parseInt(productInvField.getText()));
            modifiedProduct.setPrice(Double.parseDouble(productPriceField.getText()));
            modifiedProduct.setMax(Integer.parseInt(productMaxField.getText()));
            modifiedProduct.setMin(Integer.parseInt(productMinField.getText()));
    ...
    public void setControllerData(ObservableList<Part> partsList, 
            ObservableList<Product> productList, Product productToBeModified ){
        
        modifiedProduct = productToBeModified;
        
        System.out.println("Modified screen product part size: " + modifiedProduct.getPartsList().size());
        productsTableData = productList;
        
       
        for(int x = 0; x < modifiedProduct.getPartsList().size(); x++){
            associatedTableData.add(modifiedProduct.getPartsList().get(x));
            System.out.println("Associated table size :" + associatedTableData.size());
        }
        
        if(!partsList.isEmpty()){
        
        //set fields to product data passed by controller
        productIDField.setText(String.valueOf(modifiedProduct.getProductID()));
        productNameField.setText(modifiedProduct.getName());
        productInvField.setText(String.valueOf(modifiedProduct.getInStock()));
        ...
        //setting up the table1 data source
        tableView1.setItems( partsTableData );
      ...
      //setting up the table2 data source
        tableView2.setItems( associatedTableData );
    ...
    private boolean checkIfTextFieldsAreFilledCorrectly(){
     ...
     public double getTotalOfAllPartsPrices(){
         double total = 0;
         for(int x = 0; x < modifiedProduct.getPartsList().size(); x++){
             total = total + modifiedProduct.getPartsList().get(x).getPrice();
         }
         return total;
     }
